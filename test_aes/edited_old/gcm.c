               ...
               size_t add_len )     // length of additional AEAD data (bytes)
{
    ...
    {   
        ...
        while( iv_len > 0 ) {
            ...
            gcm_mult( ctx, (unsigned char)ctx->y, (unsigned char)ctx->y );
            iv_len -= use_len;
            p += use_len;
        }
    ...
    strncpy(ctx->base_ectr, ctx->y, sizeof(ctx->y));
...
{
    //int ret;            // our error return if the AES encrypt fails
    char ectr[16];     // counter-mode cipher output for XORing
    size_t use_len;     // byte count to process, up to 16 bytes
    size_t i;           // local loop iterator
    ...
    while( length > 0 ) {
        ...
        strncpy(ectr,ctx->y, sizeof(ctx->y));
        encrypt(ectr);
