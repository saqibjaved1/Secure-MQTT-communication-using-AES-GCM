                ...
                const uint keysize) // must be 128, 192 or 256
{
    ...
    // function was modified here to use the already available aes-128 function in the 
    // contiki library
    
    //Printing the passed information*****************************************************
   	/*
    printf("\n\nGCM Set key Function...\n");
	printf("AES Key of size ( %d ) = ", keysize);
	for(int i = 0; i< keysize; i++)
		printf("%x", *(key+i));
    */
    set_key(key); 
    encrypt(h);// This function takes in a pointer therefore dereferenced
    
    /*
	printf("\nAll '0' vector of size ( %d ) encrypted value is = ", (int)sizeof(h));
	for(int i = 0; i< sizeof(h); i++)
		printf("%x", *(h+i)); 	  
	*/
	
	 // FUNCTION WORKING CORRECTLY TILL HERE 
	//*********************************************************************************** 
...
{
    // our error return if the AES encrypt fails (REMOVED)
    ...
    //Edited the function from here by passing it to out AES **********************************
    //setting the key again and encrypting with that
    
    /*
    printf("\n\nGCM Start Function...\n");
	printf("Initialization Vector of size ( %d ) = ", (int)iv_len);
	for(int i = 0; i< iv_len; i++)
		printf("%x", *(iv+i));
	
	printf("\nCTX -> Y of size ( %d ) = ", (int)sizeof(ctx->y));
	for(int i = 0; i<sizeof(ctx->y) ; i++)
		printf("%x", ctx->y[i]);
	*/
	
    memcpy(ctx->base_ectr, ctx->y, sizeof(ctx->y));
    /*
    printf("\nCTX -> ECTR of size ( %d ) = ", (int)sizeof(ctx->base_ectr));
	for(int i = 0; i<sizeof(ctx->base_ectr) ; i++)
		printf("%x", ctx->base_ectr[i]);
    */
    encrypt(ctx->base_ectr);
    /*
    printf("\nEncrypted CTX -> ECTR of size ( %d ) = ", (int)sizeof(ctx->base_ectr));
	for(int i = 0; i<sizeof(ctx->base_ectr) ; i++)
		printf("%x", ctx->base_ectr[i]);
	*/
	//****************************************************************************************
                ...
                unsigned char *output )         // pointer to destination data
{
    ...
    while( length > 0 ) {
        ...
        for( i = 16; i > 12; i-- ) if( ++ctx->y[i - 1] != 0 ) break;//Counter being updated here

        // encrypt the context's 'y' vector under the established key**********************
        /*
        printf("\n\nGCM Update Function...\n");
		printf("Plain Text to be encrypted of size ( %d ) = ", (int)length);
		for(int k = 0; k< length; k++)
			printf("%x", *(input+k));
		*/
        memcpy(ectr,ctx->y, sizeof(ctx->y));
        /*
        printf("\nECTR of size ( %d ) = ", (int)sizeof(ectr));
		for(int k = 0; k<16 ; k++)
			printf("%x", ectr[k]);
		*/
        encrypt(ectr); // Encryption working alright
		/*
		printf("\nECTR encrypted= " );
		for(int k = 0; k<16 ; k++)
			printf("%x", ectr[k]);
                ...
                size_t tag_len )    // length, in bytes, of the tag-receiving buf
{
    ...
    if( orig_len || orig_add_len ) {
		...
		//Checking to see if Tag is calculated correctly or not***************************
        /*
        printf("\n\nGCM Finish Function...\n");
		printf("Len(A)||Len(B) = ");
		for(i = 0; i< 16; i++)
		...
		*/
        ...
        printf("\nCTX->Length processed so far 'Variable' = %d ", (int)ctx->len);
		*/	     
...
int gcm_auth_decrypt(
        ...
        unsigned char *output,          // pointer to the cipher data destination
        size_t length,          // byte length of the cipher data
        const unsigned char *tag,       // pointer to the tag to be authenticated
        size_t tag_len )        // byte length of the tag <= 16
{
    unsigned char check_tag[16];        // the tag generated and returned by decryption
